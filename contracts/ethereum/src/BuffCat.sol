    ...
    ReentrancyGuardUpgradeable
{
    ...
    event TokenAndPoolAdded(
        address token,
        address pool,
        address pairToken,
        uint256 timestamp
    );
    event TokenAndPoolRemoved(
        address token,
        address pool,
        address pairToken,
        uint256 timestamp
    ...
    function calculateTotalMultiplier(
        ...
        if (
            ...
            (effectiveAmount >= (lock.amount - acceptableAmount))
        ) {
    ...
    function calculateUserRewards(
        ...
        uint256 effectiveAmount = lock.amount - lock.withdrawn;
        uint256 effectiveAmountInUsd = (effectiveAmount *
            _currentPrice *
            USD_SCALER) / (10 ** (_tokenFeedDecimals + _tokenDecimals));
    ...
    function getTwapPrice(
        address token
    ) internal returns (uint256, uint8, uint8) {
        ...
        if (pairTokenFeedDecimals == 0) {
            pairTokenFeedDecimals = registry.decimals(
                pairToken,
                Denominations.USD
            );
        ...
        (, int256 pairTokenPrice, , , ) = registry.latestRoundData(
            pairToken,
            Denominations.USD
        );
    ...
    function removeTokenPools(
        address[] calldata _tokens
    ) external onlyAuthorized {
        for (uint256 i = 0; i < _tokens.length; i++) {
            if (_tokens[i] == address(0)) revert InvalidInput();
            TokenPool storage tokenPool = tokenPools[_tokens[i]];
            emit TokenAndPoolRemoved(
                _tokens[i],
                tokenPool.pool,
                tokenPool.pairedToken,
                block.timestamp
            );
