...
contract AdminFunctionsTests is TestSetUp {
  ...
  function testPauseUnpause() public {
    ...
    buffCat.lockAssets(address(token1), LOCK_AMOUNT, 30, LockType.FLEXIBLE, address(0));
    ...
    uint256 balanceAfterUnlocking = ((INITIAL_BALANCE - LOCK_AMOUNT) + (unlockAmount - unlockFees));
    assertEq(token1.balanceOf(user1), balanceAfterUnlocking, '1: Wrong Balance');
    ...
    assertGt(token1.balanceOf(user1), balanceAfterUnlocking, '2: Wrong Balance');
  ...
  function testSuddenBlacklist() public {
    ...
    buffCat.lockAssets(address(token2), LOCK_AMOUNT, 30, LockType.FLEXIBLE, address(0));
    ...
    assertEq(token2Balance, INITIAL_BALANCE + expectedRewards, 'Wrong balance after claiming');
    ...
    vm.expectRevert(abi.encodeWithSelector(BuffCatUpgradeable.InvalidToken.selector));
    buffCat.claimRewards(tokens, 0, 0);

    vm.stopPrank();
  }

  function testBlacklistArray() public {
    vm.startPrank(owner);

    address[] memory whitelist = buffCat.getWhitelistedTokens();
    console.log('Token Whitelist Before :-');
    for (uint256 i = 0; i < whitelist.length; i++) {
      console.log('Index %s:', i, whitelist[i]);
    }

    address[] memory blacklist = new address[](1);
    blacklist[0] = address(token6);
    buffCat.blacklistTokens(blacklist);

    uint256 length = tokensWhitelist.length;
    address lastToken = tokensWhitelist[length - 1];
    tokensWhitelist[5] = lastToken;
    tokensWhitelist.pop();

    whitelist = buffCat.getWhitelistedTokens();
    console.log('Token Whitelist Before :-');
    for (uint256 i = 0; i < whitelist.length; i++) {
      console.log('Index %s:', i, whitelist[i]);
    }

    assertEq(whitelist, tokensWhitelist, 'Wrong Token Whitelist');
