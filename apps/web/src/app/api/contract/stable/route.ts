...
export async function GET(
  request: NextRequest,
): Promise<NextResponse<StableCoinsResponse | ErrorResponse>> {
  ...
  try {
    ...
    return NextResponse.json<StableCoinsResponse>(response, { status: 200 });
  } catch (error) {
    console.error(error);
    return jsonError('Failed to fetch stable coins.', 500);
  }
}
